...
import { fetchAutomateSave, listAutomateSaves, saveAutomateSave } from '../api/automateSavesApi.js'
...
/**
 * Edit an Automate save with fill + Edit tools.
 * Saves to /api/automate-saves — never writes Editor templates or Projects.
 */
export default function AutomateSaveEditPage({ Nav }) {
  const { id: routeId } = useParams()
  const navigate = useNavigate()
  const saveId = String(routeId || '')
    .trim()
    .replace(/[^\w-]+/g, '_')
  ...
  const [saveRow, setSaveRow] = useState(null)
  const [siblings, setSiblings] = useState([])
  const [status, setStatus] = useState('Loading Automate save…')
  ...
  const siblingIndex = useMemo(() => {
    if (!siblings.length || !saveId) return -1
    return siblings.findIndex((p) => p.id === saveId)
  }, [siblings, saveId])
  ...
  const goSibling = useCallback(
    (dir) => {
      ...
      setImages({})
      setSmartCrop(null)
      navigate(`/automate-saves/${encodeURIComponent(target.id)}`)
    },
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (saveId && !sorted.some((p) => p.id === saveId) && saveRow) {
          sorted.push({
            id: saveRow.id,
            name: saveRow.name,
            teamKey: saveRow.teamKey,
            category: saveRow.category,
          })
        }
        setSiblings(sorted)
      } catch (_) {
  ...
  const apply = useCallback(
    async (force = false, { resetLayout = false } = {}) => {
      if (!api?.setPayload || !saveId || !hydrated) return
      try {
        ...
        if (force) setStatus(`Live · “${saveRow?.name || saveId}”`)
      } catch (e) {
        setStatus('Update failed: ' + (e.message || e))
      }
    },
    [api, saveId, hydrated, layoutReady, text, colors, images, saveRow?.name],
  )

  useEffect(() => {
    if (!ready || !api || !saveId) return
    let cancelled = false
    ;(async () => {
      try {
        setStatus('Loading Automate save…')
        setHydrated(false)
        setLayoutReady(false)
        const row = await fetchAutomateSave(saveId)
        if (cancelled) return
        setSaveRow(row)
        ...
        // Wait for that wipe first, then inject the Automate-save JSON.
        ...
        if (!injected?.ids?.includes?.(row.id) && !api.listTemplates?.()?.some((t) => t.id === row.id)) {
          throw new Error(`Engine did not accept Automate save “${row.id}”`)
        }
        ...
        if (!cancelled) {
          setHydrated(false)
          setStatus(e.message || 'Failed to load Automate save')
        }
      }
    })()
    return () => {
  ...
  function enterEditMode() {
    ...
    setStatus('Edit Automate save · styles & layers (Save to keep)')
  }
  ...
  function exitEditMode() {
    ...
    setStudioMode(null)
    syncFieldsFromEngine()
    setStatus('Edit closed · fill & export · Save Automate save to persist')
  }

  async function onSave() {
    if (!api?.bakeTemplate || !saveRow) return
    setSaving(true)
  ...
  async function onExport() {
    ...
    try {
      ...
      a.href = dataUrl
      a.download = `autosave_${saveId || 'export'}.png`
      a.click()
  ...
  async function persistProcessedImage(slotKey, src) {
    ...
    try {
      const url = await uploadDataUrlToS3(src, { folder: `automate-saves/${slotKey}` })
  ...
  const title = saveRow?.name || saveId || 'Automate save'
  const posLabel =
        ...
        <header className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-line bg-panel px-3">
          <div className="min-w-0">
            ...
            <div className="truncate text-[11px] text-muted">
              {posLabel ? `${posLabel} · ` : ''}
              {editMode
                ? 'Edit mode · styles & layers · Save to keep'
                : 'Fill copy · drag on canvas · Save Automate save to persist'}
            </div>
          </div>
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  setLayoutReady(false)
                  setStatus('Layout reset to saved bake')
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          <aside className="flex w-[300px] shrink-0 flex-col overflow-y-auto border-r border-line bg-panel p-3">
            <p className="mb-3 text-[11px] text-muted">
              Same tools as Automate. Saves to <strong className="text-dim">Automate saves</strong>{' '}
              only — not Editor templates or Projects.
            </p>
            ...
            <section className="mb-3 rounded-xl border border-line bg-inset/40 p-3.5 shadow-sm">
              ...
              {!fields.length ? (
                <p className="text-[11px] text-muted">No text fields on this save yet.</p>
              ) : null}
            </section>
            ...
            <section className="mb-3 rounded-xl border border-line bg-inset/40 p-3.5 shadow-sm">
              ...
              {!imageSlots.length ? (
                <p className="text-[11px] text-muted">No image slots on this saveRow.</p>
              ) : null}
            </section>
          </aside>
              ...
              <aside className="flex min-h-0 w-[300px] shrink-0 flex-col overflow-hidden border-l border-line bg-panel">
                ...
                <div className="shrink-0 space-y-2 border-t border-line p-3.5">
                  ...
                  <p className="text-[10px] leading-snug text-muted">
                    Use <strong className="text-dim">Save</strong> to write changes to Automate
                    saves (not templates or Projects).
                  </p>
                </div>
              </aside>
            ) : null}
